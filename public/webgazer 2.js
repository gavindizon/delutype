(function (window, undefined) {
    ...
    var curTrackerMap = {
        clmtrackr: function () {
            return new webgazer.tracker.ClmGaze();
        },
        trackingjs: function () {
            return new webgazer.tracker.TrackingjsGaze();
        },
        js_objectdetect: function () {
            return new webgazer.tracker.Js_objectdetectGaze();
        },
    };
    var regressionMap = {
        ridge: function () {
        ...
        linear: function () {
            return new webgazer.reg.LinearReg();
        },
    };
    ...
    function checkEyesInValidationBox() {
        var eyesObjs = curTracker.getEyePatches(
            videoElementCanvas,
            webgazer.params.imgWidth,
            ...
            navigator.mediaDevices.getUserMedia = function (constraints) {
                ...
                if (!getUserMedia) {
                    return Promise.reject(
                        new Error(
                            "Unfortunately, your browser does not support access to the webcam through the getUserMedia API. Try to use Google Chrome, Mozilla Firefox, Opera, or Microsoft Edge instead."
                        )
                    );
                }
